...
class StorageManager:
    def __init__(self, name, load_session, load_episode, device, stage):
        ...
        base_path = os.getenv('DRLNAV_BASE_PATH') or '/workspace'
        ...
        # Keep pretrained example models loadable from the original repo path:
        #   src/turtlebot3_drl/model/examples/...
        if load_session and 'examples' in load_session:
            self.machine_dir = os.path.join(base_path, 'src', 'turtlebot3_drl', 'model')
        else:
            # Save our own trained checkpoints directly into:
            #   models/<algorithm>/<run_name>/
            self.machine_dir = os.path.join(base_path, 'models', self.name)
            os.makedirs(self.machine_dir, exist_ok=True)

        self.session_dir = os.path.join(self.machine_dir, self.session) if self.session else self.machine_dir

    def new_session_dir(self, stage):
        i = 0
        session_name = f"{self.name}_{i}_stage_{stage}"
        session_dir = os.path.join(self.machine_dir, session_name)
